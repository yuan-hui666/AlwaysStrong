...
**Fixes**
- **Action no longer reports "keybox ok" on a fresh install without internet.** The installer seeds the upstream demo keybox as a placeholder so the engine has something to load, and Action only checked that a keybox-looking file existed. The placeholder is now marked at install (by hash), Action says `placeholder keybox — fetch failed / no internet? retried hourly` until the real one arrives, the log collector flags it, and the marker clears itself as soon as the fetch, a WebUI import or any other replacement lands.
